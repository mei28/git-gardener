...
3. `list` - worktree一覧表示
4. `config` - 設定の確認・編集
5. `clean` - 不要なworktreeの削除
6. `pull-all` - 全worktreeでgit pull
7. `tui` - 対話式UI（後期実装）
8. `go` - worktreeへの移動（低優先度）

## 設定ファイル

`.git/gardener.toml` に各リポジトリ固有の設定を保存します。

## 開発進捗とTODO

### 現在のフェーズ: PoC (Proof of Concept)

#### 完了したタスク
- [x] プロジェクト仕様書作成
- [x] Rustプロジェクト初期化
- [x] 基本的なCLI構造の実装
- [x] initコマンドの実装（設定ファイル生成）
- [x] addコマンドの基本実装（worktree作成、ブランチ切り替え）
- [x] listコマンドの基本実装（worktree一覧表示）
- [x] configコマンドのview機能実装

#### 次のフェーズで実装予定
- [ ] cleanコマンドの実装
- [ ] pull-allコマンドの実装
- [ ] フック機構（post_create）の実装
- [ ] 並列処理の実装
- [ ] TUIの実装
- [ ] goコマンドの実装（低優先度）
...
- clap v4でCLIを構築
- エラーハンドリングにはanyhowとthiserrorを使用
