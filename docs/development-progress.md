...
- [x] 古いworktree判定の実装（コミット日時ベース）

#### 🔵 Refactor（リファクタリング）
- [x] マージ判定ロジックのcleanコマンドへの統合
- [x] stale判定ロジックのcleanコマンドへの統合
- [x] エラーハンドリングの統一
...
#### 🔴 Red（失敗するテスト）
- [x] 基本的なconfig set実行テスト
- [x] 実際の設定ファイル変更テスト  
- [x] 無効なキーに対するエラーハンドリングテスト

#### 🟢 Green（最小限の実装）
- [x] 仮実装：成功メッセージの表示
- [x] 実装：設定ファイルの読み込み・変更・保存機能
- [x] 三角測量：複数の設定キーパターンへの対応

#### 🔵 Refactor（リファクタリング）
- [x] エラーハンドリングの統一
- [x] 設定キーの構造化対応（defaults.root_dir等）
